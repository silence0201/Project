...
@interface SIMetionTextView ()<UITextViewDelegate>
...
@implementation SIMetionTextView
...
- (NSString *)renderedString {
    ...
    for (int i = 0; i < self.quoteArray.count; i ++) {
        ...
        for (int j = i+1; j < self.quoteArray.count; j ++) {
            SIQuote *restQuote = self.quoteArray[j];
...
- (void)setNeedsRefreshQuotes {
    for (SIQuote *qoute in self.quoteArray) {
...
- (void)removeQuote:(SIQuote *)quote {
...
- (void)disableQuote:(SIQuote *)quote {
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    self.isInit = NO;
    if (range.length == 1) {
        SIQuote *quote = [self quoteInRange:range];
        if (quote) {
...
- (void)textViewDidChangeSelection:(UITextView *)textView {
    ...
    SIQuote *quote = [self quoteInRange:self.selectedRange];
    if (quote) {
...
- (void)setBackgroundFrameForQuote:(SIQuote *)quote {
...
- (SIQuote *)quoteInRange:(NSRange)range {
...
- (NSArray *)framesOfQuote:(SIQuote *)quote {
    return [self framesFromRange:[self rangeOfQuote:quote]];
}

- (NSRange)rangeOfQuote:(SIQuote *)quote {
    ...
    @autoreleasepool {
        ...
        if (ranges.count > 1) {
            ...
            for (int i = 0; i < self.quoteArray.count; i ++) {
                SIQuote *tempQuote = self.quoteArray[i];
